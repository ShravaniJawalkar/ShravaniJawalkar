...
@Aspect
@Configuration
public class NewTermPoint {
	Logger log = LoggerFactory.getLogger(this.getClass());

	@Before("com.example.AOP.AspectProg.CommonConfig.allLayer()")
	public void beforExe(JoinPoint joinPoint) {
		log.info("User Access");
		log.info("Allowing user Access {}" + joinPoint);
	}

	@AfterReturning(value = "com.example.AOP.AspectProg.CommonConfig.beanContainWithDao()", returning = "result")
	...
	@After("com.example.AOP.AspectProg.CommonConfig.dataLayerExecution()")
	public void after(JoinPoint joinPoint) {
		log.info("after execution {}", joinPoint);
	}

	@Around("com.example.AOP.AspectProg.CommonConfig.retriveBusiness()")
